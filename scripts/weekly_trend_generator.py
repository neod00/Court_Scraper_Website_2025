...
TREND_PROMPT = """당신은 법원 회생·파산 자산매각 시장을 분석하는 **프리미엄 매거진 에디터**입니다.
독자가 매주 정기 구독하고 싶어지는, 시각적으로 잘 정리되고 인사이트가 풍부한 리포트를 작성합니다.
...
2. "full_report": 아래 구조를 **정확히** 따른 마크다운 리포트 (2000~3500자).

반드시 아래 형식과 구조를 **그대로** 따르세요:

```
## 📊 {week_start} ~ {week_end} 주간 자산매각 시장 동향

> 💡 **에디터 한 줄 요약**: (이번 주 시장을 한 문장으로 요약)

---

### 📈 1. 이번 주 시장 개요

이번 주 법원 자산매각 시장에... (전주 대비 증감률, 전반적 흐름을 2~3문단으로 서술)
...
(부동산 카테고리의 이번 주 주요 트렌드, 눈에 띄는 매물, 지역 분포 등을 분석. 없으면 간략하게.)

### 🚗 3. 차량/동산 매각 동향

(차량 카테고리의 이번 주 주요 트렌드, 차종, 가격대 등을 분석. 없으면 간략하게.)

### 📦 4. 기타 자산 동향

(채권, 주식, 특허, 기계 등 기타 카테고리를 간략히 분석.)

---

### 🏆 5. 이번 주 에디터 PICK: 주목 매물 TOP 3

각 매물을 아래 형식으로 작성해주세요:

**🥇 1위: (매물명 또는 요약 제목)**
- 📍 관할: (법원명)
- 💰 최저매각가: (금액)
- ✨ 주목 포인트: (왜 주목할 만한지 1~2줄)
...
> 📌 **주의사항**: 본 리포트는 AI가 공고문 데이터를 기반으로 자동 생성한 분석으로, 투자 권유가 아닙니다. 실제 투자 결정 시에는 반드시 원본 공고문을 확인하고 전문가와 상담하시기 바랍니다.
```
...
def generate_weekly_report(data: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        response = openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "당신은 법원 자산매각 시장 전문 분석가이자 프리미엄 매거진 에디터입니다. 수집된 데이터만을 기반으로 객관적이고 시각적으로 잘 정리된 분석을 제공합니다. 마크다운 형식을 완벽하게 활용하여 표, 이모지, 체크리스트, 인용문 등을 적극적으로 사용합니다. 반드시 유효한 JSON으로만 응답하세요."},
