...
const CORRECTION_LOG: { date: string; summary: string; detail: string }[] = [
    {
        date: '2026-09-10',
        summary: '주간 칼럼·월간 리포트 자동 발행 도입(자동 작성 고지 표시)',
        detail:
            '주간 데이터 칼럼과 월간 리포트의 노트를 로옥션이 수집한 공고 집계만을 입력으로 자동 작성하고, 본문 수치가 집계와 일치하는지와 금지 표현·개인명·원인 단정이 없는지 검증한 뒤 발행하는 방식으로 바꿨습니다. 해당 글에는 자동 작성 고지와 작성자(로옥션 데이터 데스크)를 표시하며, 운영자가 사후 검토해 오류를 정정합니다. 편집·데이터 운영 원칙의 AI 활용 범위·발행 주기·AI 사용 고지를 이에 맞게 고쳤습니다.',
    },
    {
        date: '2026-09-08',
    ...
    return (
        <article className="max-w-4xl mx-auto bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-10">
            ...
            <p className="text-gray-600 leading-relaxed mb-8">
                로옥션(LawAuction)은 1인이 운영하는 민간 정보 서비스입니다. 편집 글·용어사전·FAQ는 운영자가 직접 작성하고
                공개 전에 원문과 대조합니다. 주간 칼럼과 월간 리포트의 노트는 수집 공고의 집계를 바탕으로 자동 작성·검증해
                발행하고 운영자가 사후 검토합니다. 법원 공개자료를 사용자가 확인하기 쉬운 순서로 정리하되,
            ...
            <div className="space-y-8 text-gray-700 leading-relaxed">
                <section>
                    ...
                    <ol className="list-decimal pl-6 space-y-2">
                        ...
                        <li>
                            편집 글에서 AI는 초안 정리와 누락 점검에만 사용하고 공개 전 사람이 원문과 대조합니다. 자동 작성하는
                            주간 칼럼·월간 리포트 노트는 집계 수치만을 입력으로 쓰고, 본문 수치가 집계와 일치하는지와 금지
                            표현·개인명·원인 단정이 없는지를 프로그램이 검증한 뒤 발행하며, 사람은 발행 후 검토합니다. 자동 작성한
                            글에는 그 사실을 표시합니다.
                        </li>
                ...
                <section>
                    ...
                    <dl className="space-y-3">
                        <div>
                            <dt className="font-semibold text-gray-900">주간 데이터 칼럼 — 매주 토요일 자동 발행, 사후 검토</dt>
                            <dd>
                                직전 주(월~금) 공고 집계로 노트를 자동 작성하고 검증을 통과한 주차만 발행합니다. 노트가
                                200자 미만이거나 검증을 통과하지 못한 주차는 발행하지 않으며 검색엔진에도 노출하지 않습니다.
                                운영자는 발행된 칼럼을 사후에 읽고 오류가 있으면 정정하거나 내립니다.
                            </dd>
                        </div>
                        <div>
                            <dt className="font-semibold text-gray-900">월간 리포트 — 매월 2일 자동 발행, 사후 검토</dt>
                            <dd>
                                직전 달 집계로 표와 노트를 만들어 검증을 통과한 달만 공개합니다(과거 달은 주 1편씩 순차 발행).
                                노트가 300자 미만이거나 검증을 통과하지 못한 달은 공개하지 않습니다. 운영자는 발행 후 검토합니다.
                            </dd>
                        </div>
                        <div>
